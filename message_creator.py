...
class MessageCreator:
	...
	def generate_messages(self, requested_index, requested_db, record_size, pir_xor = False):
		if not pir_xor:
			indexes = self._generate_random_indexes(requested_index, record_size)
			real_message = self.client.package_message(requested_index, requested_db, pir_xor)
			db_index = {}
			...
			current_db = 0
			for i in indexes:
				db_index[current_db].append(self.client.package_message(i, current_db, pir_xor))
